...
// Area로 쌓아 보여준다. Zone/Mailbox/AI 챗봇/Contact 모달 중 하나라도 열려
// 있으면 통째로 숨겨서 모달과 겹치지 않게 한다.
export default function FloatingActionArea({ gameStarted }) {
  const { activeZone, mailboxModalOpen } = useGame();
  const { npcChatOpen } = useNpcChat();
  const { chatOpen } = useChat();

  const anyModalOpen = Boolean(activeZone || mailboxModalOpen || npcChatOpen || chatOpen);
